...
{
    PacketMonitorDataSourceKindAll = 0,
    PacketMonitorDataSourceKindNetworkInterface = 1,

    // Assuming from pktmon output...
    PacketMonitorDataSourceKindFilterDriver = 2,
    PacketMonitorDataSourceKindProtocolDriver = 3,
    PacketMonitorDataSourceKindApplicationProtocolDriver = 4,
    PacketMonitorDataSourceKindVSwitch = 5,
    PacketMonitorDataSourceKindOther = 6
...
typedef
HRESULT
(WINAPI* PacketMonitorInitialize)(
...
typedef
VOID
(WINAPI* PacketMonitorUninitialize)(
    _In_ PACKETMONITOR_HANDLE handle
    );

typedef
HRESULT
(WINAPI* PacketMonitorEnumDataSources)(
    _In_ PACKETMONITOR_HANDLE handle,
...
typedef
HRESULT
(WINAPI* PacketMonitorCreateLiveSession)(
    _In_ PACKETMONITOR_HANDLE handle,
    _In_ PCWSTR name,
...
typedef
VOID
(WINAPI* PacketMonitorCloseSessionHandle)(
    _In_ PACKETMONITOR_SESSION session
    );

typedef
HRESULT
(WINAPI* PacketMonitorAddSingleDataSourceToSession)(
    _In_ PACKETMONITOR_SESSION session,
...
typedef
HRESULT
(WINAPI* PacketMonitorAddCaptureConstraint)(
    _In_ PACKETMONITOR_SESSION session,
...
typedef
HRESULT
(WINAPI* PacketMonitorSetSessionActive)(
    _In_ PACKETMONITOR_SESSION session,
    _In_ BOOLEAN active
    );
...
typedef
HRESULT
(WINAPI* PacketMonitorCreateRealtimeStream)(
    _In_ PACKETMONITOR_HANDLE handle,
...
typedef
VOID
(WINAPI* PacketMonitorCloseRealtimeStream)(
    _In_ PACKETMONITOR_REALTIME_STREAM realtimeStream
);

typedef
HRESULT
(WINAPI* PacketMonitorAttachOutputToSession)(
    _In_ PACKETMONITOR_SESSION session,
    _In_ VOID* outputHandle
);

FARPROC GetPktmonAPI(const char* apiName)
{
    HMODULE hModule = LoadLibraryA("pktmonapi.dll");
    if (hModule == NULL)
    {
        throw new std::runtime_error("Failed to load pktmonapi.dll");
    }
    FARPROC procAddress = GetProcAddress(hModule, apiName);
    if (procAddress == NULL)
    {
        throw new std::runtime_error(
            std::format("Failed to get address of API function: {}", apiName));
    }
    return procAddress;
}